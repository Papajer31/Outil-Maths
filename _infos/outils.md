...
Dans `Entrée phonémique`, l’ordre des graphies et leur classement `fréquentes / rares` sont des données fixes du référentiel `shared/phonology-targets.js`. Ils ne dépendent ni du niveau de mots choisi, ni du pool disponible dans un outil, ni des graphies déjà cochées. Cocher ou décocher une graphie ne modifie donc jamais sa position ni sa catégorie.

La banque phonologique utilise désormais le format `mot|codes|syllabation|niveau|score_regularite`. Le niveau manuel est la donnée principale : `CP ⊂ CE1 ⊂ CE2 ⊂ CM`; `X` reste hors sélection automatique. Les outils filtrent d’abord par ce niveau, puis appliquent uniquement leurs contraintes explicites (cible phonémique ou graphémique, graphies activées, longueur, image, nombre de syllabes, etc.). Le score de régularité G-P 0–100 ne peut jamais exclure un mot : lors d’un tirage, son poids est simplement égal à son score (un mot à 100 est donc deux fois plus probable qu’un mot à 50). L’ancienne familiarité et les catégories `Simple / Normal / Complexe` ne participent plus à la sélection.
