-- =========================================================
-- PATCH 02 — Catalogue : masquer/afficher dans Exploration
-- =========================================================

create table if not exists public.catalog_activity_visibility (
...
create index if not exists catalog_activity_visibility_space_visible_idx
...
drop policy if exists catalog_activity_visibility_select_own on public.catalog_activity_visibility;
drop policy if exists catalog_activity_visibility_insert_own on public.catalog_activity_visibility;
drop policy if exists catalog_activity_visibility_update_own on public.catalog_activity_visibility;
drop policy if exists catalog_activity_visibility_delete_own on public.catalog_activity_visibility;

create policy catalog_activity_visibility_select_own
on public.catalog_activity_visibility
for select to authenticated
using (exists (select 1 from public.teacher_spaces ts where ts.id = catalog_activity_visibility.teacher_space_id and ts.owner_user_id = auth.uid()));

create policy catalog_activity_visibility_insert_own
on public.catalog_activity_visibility
for insert to authenticated
with check (exists (select 1 from public.teacher_spaces ts where ts.id = catalog_activity_visibility.teacher_space_id and ts.owner_user_id = auth.uid()));
...
returns table (catalog_activity_id text, is_visible boolean, updated_at timestamptz)
language sql
stable
security definer
