...
export function normalizeImageZoom(value){
  const number = Number(value);
  return round(clamp(Number.isFinite(number) ? number : 1, IMAGE_ZOOM_MIN, IMAGE_ZOOM_MAX), 2);
}

/**
 * Position logique, totalement indépendante de la résolution.
 * -1 = extrémité gauche/haute disponible, 0 = centre, +1 = extrémité droite/basse.
 * Le rendu convertit ensuite cette valeur dans l'espace réellement disponible.
 */
...
// Conservé pour migrer proprement les états Image de l'ancien moteur.
...
function legacyOffsetToPosition(value, zoom){
  const maxOffset = getMaxImageOffset(zoom);
  if (maxOffset <= 0) return 0;
  return normalizeImagePosition((Number(value) || 0) / maxOffset);
}
...
export function normalizeImageState(rawState = {}){
  const source = normalizeImageSource(rawState.source);
  const hasImage = Boolean(source);
  const zoom = hasImage ? normalizeImageZoom(rawState.zoom) : 1;
  const hasPositionX = Number.isFinite(Number(rawState.positionX));
  const hasPositionY = Number.isFinite(Number(rawState.positionY));
  const positionX = zoom === 1
    ? 0
    : normalizeImagePosition(hasPositionX ? rawState.positionX : legacyOffsetToPosition(rawState.offsetX, zoom));
  const positionY = zoom === 1
    ? 0
    : normalizeImagePosition(hasPositionY ? rawState.positionY : legacyOffsetToPosition(rawState.offsetY, zoom));
  ...
  return {
    ...
    zoom,
    positionX: hasImage ? positionX : 0,
    positionY: hasImage ? positionY : 0,
...
export function applyImageAction({ action, payload = {}, state } = {}){
    ...
    return {
      patch: {
        state: normalizeImageState({
          ...
          positionX: currentState.zoom === 1 ? 0 : currentState.positionX,
          positionY: currentState.zoom === 1 ? 0 : currentState.positionY,
          updatedAt: Date.now()
        })
      }
    };
    ...
    return {
      patch: {
        state: normalizeImageState({
          ...currentState,
          zoom: nextZoom,
          positionX: nextZoom === 1 ? 0 : currentState.positionX,
          positionY: nextZoom === 1 ? 0 : currentState.positionY,
          updatedAt: Date.now()
        })
      }
    };
  ...
  if (safeAction === "set-position") {
    return {
      patch: {
        state: normalizeImageState({
          ...currentState,
          positionX: currentState.zoom === 1 ? 0 : normalizeImagePosition(payload?.positionX),
          positionY: currentState.zoom === 1 ? 0 : normalizeImagePosition(payload?.positionY),
          updatedAt: Date.now()
        })
      }
    };
  ...
  // Compatibilité transitoire avec un éventuel message encore émis par une ancienne popup.
  if (safeAction === "set-offset") {
    return {
      patch: {
        state: normalizeImageState({
          ...currentState,
          positionX: legacyOffsetToPosition(payload?.offsetX, currentState.zoom),
          positionY: legacyOffsetToPosition(payload?.offsetY, currentState.zoom),
          updatedAt: Date.now()
        })
      }
    };
