...
export function pickQuestion(settings = {}, {
  ...
  let selected = pickWeightedWords(source, cfg.wordCount);
  ...
  if (avoidKey && key === avoidKey && pool.length > cfg.wordCount) {
    ...
    const replacement = pickWeightedWords(outside, 1)[0];
...
function pickWeightedWords(pool, count) {
  return pickPhonologyWordsByRegularity(pool, count);
}
