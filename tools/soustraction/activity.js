...
const SUB_IDS = {
  ...
  responseWrap: "sub_response_wrap",
  keypadSlot: "sub_keypad_slot"
};

const SUB_KEYPAD_DATA_ATTRIBUTE = "data-sub-numeric-key";
...
function renderShell(state) {
  ...
  container.innerHTML = renderCalcFamilyShell({
    ...
    instructionHtml: renderToolInstruction({ id: SUB_IDS.instruction }),
    expressionId: SUB_IDS.expression,
    responseWrapId: SUB_IDS.responseWrap,
    keypadSlotId: SUB_IDS.keypadSlot,
    rootClassName: `sub-root${state.showResponseBox ? " sub-root--boxed" : " sub-root--free"}`,
    stageClassName: "sub-stage",
    equationClassName: "sub-equation",
    expressionClassName: "sub-expression",
    equalsClassName: "sub-equals",
    responseWrapClassName: "sub-response-wrap",
    keypadSlotClassName: "sub-keypad-slot",
    keypadRootClassName: "sub-keypad",
    keypadButtonClassName: "sub-keypad-button",
    keypadClearButtonClassName: "sub-keypad-button--clear",
    keypadDataAttribute: SUB_KEYPAD_DATA_ATTRIBUTE,
    keypadAriaLabel: "Clavier numérique"
  });

  const refs = getCalcFamilyShellRefs(container, {
    instructionId: SUB_IDS.instruction,
    expressionId: SUB_IDS.expression,
    responseWrapId: SUB_IDS.responseWrap,
    keypadSlotId: SUB_IDS.keypadSlot
  });
...
function renderResponseArea(state) {
  ...
  destroyAnswerControl(state);
  state.responseWrap.className = "tool-answer-panel calc-family-response-wrap sub-response-wrap";
  state.responseWrap.innerHTML = "";

  state.answerControl = createNumericAnswerControl({
    id: "sub_response_input",
    className: "calc-family-response-input sub-response-input",
    ariaLabel: "Réponse",
...
function bindKeypadEvents(state) {
  bindCalcFamilyKeypadEvents({
    state,
    dataAttribute: SUB_KEYPAD_DATA_ATTRIBUTE,
...
function syncKeypadVisibility(state) {
  syncCalcFamilyKeypadVisibility(state, {
    hiddenClassName: "sub-keypad-slot--hidden"
  });
}
...
function renderDisplayedResponse(state) {
  ...
  state.responseWrap.classList.toggle("sub-response-wrap--correct", evaluation.isCorrect === true);
  state.responseWrap.classList.toggle("sub-response-wrap--incorrect", showStudentAnswer && evaluation.isCorrect !== true);
  destroyAnswerControl(state);
  state.responseWrap.innerHTML = renderNumericAnswerDisplayMarkup(snapshot?.value ?? "", {
    className: `calc-family-response-input sub-response-input${showStudentAnswer ? (evaluation.isCorrect ? " calc-family-response-input--correct sub-response-input--correct" : " calc-family-response-input--incorrect sub-response-input--incorrect") : (!evaluation.isCorrect ? " calc-family-response-input--correction" : " calc-family-response-input--correct sub-response-input--correct")}`,
...
function injectStyles() {
  ...
  if (document.querySelector(`link[data-sub-activity-style="${href}"]`)) return;

  const link = document.createElement("link");
  link.rel = "stylesheet";
  link.href = href;
  link.dataset.subActivityStyle = href;
