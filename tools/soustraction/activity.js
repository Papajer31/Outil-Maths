...
function renderShell(state) {
  ...
  container.innerHTML = `
    <div class="tool-runtime sub-root${state.showResponseBox ? " sub-root--boxed" : " sub-root--free"}">
      ${renderToolInstruction({ id: "sub_instruction" })}
      <div class="tool-stage sub-stage">
        <div class="tool-answer-row sub-equation">
          <div class="tool-big tool-question sub-expression" id="sub_expression"></div>
          ${state.showResponseBox ? `
            <div class="tool-big sub-equals">=</div>
            <div class="tool-answer-panel sub-response-wrap" id="sub_response_wrap"></div>
          ` : ""}
        </div>
      </div>
    </div>
  ...
  state.root = container.querySelector(".sub-root");
  state.instructionEl = container.querySelector("#sub_instruction");
  state.exprEl = container.querySelector("#sub_expression");
  state.responseWrap = container.querySelector("#sub_response_wrap");
  state.input = null;
...
function renderDisplayedResponse(state) {
  ...
  state.responseWrap.innerHTML = renderNumericAnswerDisplayMarkup(snapshot?.value ?? "", {
    className: `sub-response-input${showStudentAnswer ? (evaluation.isCorrect ? " sub-response-input--correct" : " sub-response-input--incorrect") : ""}`,
...
function syncCompactClass(state) {
  ...
  scheduleCalcLayoutFit({
    root: state.root,
    equationEl: state.root.querySelector(".sub-equation"),
    expressionEl: state.exprEl,
    equalsEl: state.root.querySelector(".sub-equals"),
