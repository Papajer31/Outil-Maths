...
function loadNextQuestion(state, context = {}) {
  ...
  if (!nextQuestion) {
    throw new Error("Impossible de générer une question pour Nombre cible avec ces réglages.");
  }
...
function updateInstructionDisplay(state) {
  const fallback = isTargetedQuestion(state.currentQuestion)
    ? "Utilise tous ces nombres pour obtenir le nombre cible."
    : isClassicChallengeQuestion(state.currentQuestion)
      ? "Atteins le nombre cible avec les nombres proposés."
      : "Trouve les boites qui permettent d’obtenir le nombre cible.";
