...
const GLOBAL_MIN = 1;
export const TOOL_THEME_ID = "picbille";
export const TOOL_THEME_LABEL = "Picbille";
...
const PICBILLE_HUNDRED_ASSET_URL = new URL("../../shared/tool-assets/images/representation/cent.webp", import.meta.url).href;
const PICBILLE_HUNDRED_W = 120 * PICBILLE_SCALE;
const PICBILLE_HUNDRED_H = PICBILLE_HUNDRED_W * (519 / 608);
const PICBILLE_HUNDRED_GAP = 12 * PICBILLE_SCALE;
const PICBILLE_HUNDRED_ROW_GAP = 10 * PICBILLE_SCALE;
const PICBILLE_SECTION_GAP = 18 * PICBILLE_SCALE;
...
export function renderRepresentationPieceSvg(themeId, kind, options = {}) {
  ...
  switch (safeThemeId) {
    case "picbille":
      if (safeKind === "hundreds") return renderPicbillePieceSvg("hundreds");
...
function buildDisplayDecompositionForQuestion(settings, direction, themeId, decomposition) {
  ...
  // "Quelques" dizaines libres : on limite volontairement à deux dizaines
  // pour éviter les représentations trop chargées en unités.
...
function renderPicbilleSvg(value, options = {}) {
  const safeValue = clampInt(value, 1, TOOL_MAX);
  const displayBuild = getPicbilleDisplayBuild(safeValue, options);
  ...
  const tenGroups = getPicbilleTenGroupsForCount(tens, ones + looseTens * 10);
  const tenStackHeight = computePicbilleStackHeight(tenGroups);
  const tensBlockWidth = tenGroups.length ? tenStackHeight : 0;
  const tensBlockHeight = tenGroups.length ? PICBILLE_TEN_W : 0;
  const looseTensLayout = getPicbilleLooseTensLayout(looseTens);
  const useLooseUnitsLayout = options.allowLooseTens === true;
  const onesLayout = getPicbilleVerticalUnitsLayout(ones, { twoColumns: useLooseUnitsLayout });
  const hundredsRows = getPicbilleHundredRows(hundreds);
  const hundredsHeight = computePicbilleHundredsRowsHeight(hundredsRows);
  const hundredsWidth = Math.max(0, ...hundredsRows.map((count) => computePicbilleHundredsRowWidth(count)));

  const sections = [
    hundredsRows.length ? { kind: "hundreds", width: hundredsWidth, height: hundredsHeight } : null,
    tenGroups.length ? { kind: "tens", width: tensBlockWidth, height: tensBlockHeight } : null,
    looseTensLayout.count ? { kind: "looseTens", width: looseTensLayout.width, height: looseTensLayout.height } : null,
    onesLayout.count ? { kind: "ones", width: onesLayout.width, height: onesLayout.height } : null
  ].filter(Boolean);
  const contentWidth = sections.reduce((sum, section, index) => {
    return sum + section.width + (index > 0 ? PICBILLE_SECTION_GAP : 0);
  }, 0);
  const contentHeight = Math.max(1, ...sections.map((section) => section.height));
  const width = Math.ceil(contentWidth + PICBILLE_SIDE_PAD * 2);
  const height = Math.ceil(contentHeight + PICBILLE_TOP_PAD + PICBILLE_BOTTOM_PAD);
  const baselineY = PICBILLE_TOP_PAD + contentHeight;
  let cursorX = PICBILLE_SIDE_PAD;
  const bodyParts = [];

  sections.forEach((section, index) => {
    if (index > 0) cursorX += PICBILLE_SECTION_GAP;
    const topY = baselineY - section.height;

    if (section.kind === "hundreds") {
      bodyParts.push(renderPicbilleHundredsRows(hundredsRows, cursorX + section.width / 2, topY));
    } else if (section.kind === "tens") {
      bodyParts.push(renderRotatedPicbilleTensBlock(tenGroups, cursorX, topY));
    } else if (section.kind === "looseTens") {
      bodyParts.push(renderPicbilleLooseTensBlocks(looseTens, cursorX, topY));
    } else if (section.kind === "ones") {
      bodyParts.push(renderPicbilleVerticalUnits(ones, cursorX, topY, { twoColumns: useLooseUnitsLayout }));
    }
...
function getPicbilleDisplayBuild(value, options = {}) {
  const decomposition = decomposeValue(value);
...
function getPicbilleHundredRows(count) {
  const rows = [];
  let remaining = clampInt(count, 0, 9);
  while (remaining > 0) {
...
function computePicbilleHundredsRowsHeight(rows) {
  if (!rows.length) return 0;
  return rows.length * PICBILLE_HUNDRED_H + Math.max(0, rows.length - 1) * PICBILLE_HUNDRED_ROW_GAP;
}

function computePicbilleHundredsRowWidth(count) {
  if (count <= 0) return 0;
  return count * PICBILLE_HUNDRED_W + Math.max(0, count - 1) * PICBILLE_HUNDRED_GAP;
}

function renderPicbilleHundredsRows(rows, centerX, startY) {
  let cursorY = startY;
  return rows.map((count, rowIndex) => {
    const rowWidth = computePicbilleHundredsRowWidth(count);
    let cursorX = centerX - rowWidth / 2;
    const body = Array.from({ length: count }, () => {
      const x = cursorX;
      cursorX += PICBILLE_HUNDRED_W + PICBILLE_HUNDRED_GAP;
      return renderImageAsset({
        x,
        y: cursorY,
        width: PICBILLE_HUNDRED_W,
        height: PICBILLE_HUNDRED_H,
        href: PICBILLE_HUNDRED_ASSET_URL
      });
    }).join("");
    cursorY += PICBILLE_HUNDRED_H + (rowIndex < rows.length - 1 ? PICBILLE_HUNDRED_ROW_GAP : 0);
    return body;
  }).join("");
}

function getPicbilleUnitRows(count) {
  const rows = [];
  let remaining = clampInt(count, 0, 99);
  while (remaining > 0) {
...
function computePicbilleUnitsRowsHeight(rows) {
  if (!rows.length) return 0;
  return rows.length * PICBILLE_UNIT_SIZE + Math.max(0, rows.length - 1) * PICBILLE_UNIT_ROW_GAP;
}

function computePicbilleUnitsRowWidth(count) {
  if (count <= 0) return 0;
  let width = count * PICBILLE_UNIT_SIZE;
  for (let index = 0; index < count - 1; index += 1) {
    width += index === 4 ? PICBILLE_UNIT_GROUP_GAP : PICBILLE_UNIT_GAP;
  }
  return width;
}

function renderPicbilleUnitsRows(rows, centerX, startY) {
  let cursorY = startY;
  return rows.map((count, rowIndex) => {
    const rowWidth = computePicbilleUnitsRowWidth(count);
    let cursorX = centerX - rowWidth / 2;
    const body = Array.from({ length: count }, (_, index) => {
      const x = cursorX;
      cursorX += PICBILLE_UNIT_SIZE;
      if (index < count - 1) {
        cursorX += index === 4 ? PICBILLE_UNIT_GROUP_GAP : PICBILLE_UNIT_GAP;
      }
      const cx = x + PICBILLE_UNIT_SIZE / 2;
      const cy = cursorY + PICBILLE_UNIT_SIZE / 2;
    ...
    cursorY += PICBILLE_UNIT_SIZE + (rowIndex < rows.length - 1 ? PICBILLE_UNIT_ROW_GAP : 0);
    return body;
  }).join("");
}

function renderRotatedPicbilleTensBlock(groups, x, y) {
  const stackHeight = computePicbilleStackHeight(groups);
  if (!stackHeight) return "";

  const tenYs = buildPicbilleTenYs(groups, stackHeight);
  const body = tenYs.map((tenY) => renderInlineAsset({
    x: 0,
    y: tenY,
  ...
  return `
    <g transform="translate(${round(x + stackHeight)} ${round(y)}) rotate(90)">
      ${body}
    </g>
  `;
}

function getPicbilleLooseTensLayout(count) {
  const safeCount = clampInt(count, 0, 9);
  if (!safeCount) return { count: 0, width: 0, height: 0 };
  const blockWidth = computePicbilleLooseTenBlockWidth();
  const blockHeight = computePicbilleLooseTenBlockHeight();
  return {
    count: safeCount,
    width: safeCount * blockWidth + Math.max(0, safeCount - 1) * PICBILLE_SMALL_GROUP_GAP,
    height: blockHeight,
    blockWidth,
    blockHeight
  };
...
function computePicbilleLooseTenBlockWidth() {
  return PICBILLE_UNIT_SIZE * 2 + PICBILLE_UNIT_GAP;
}

function computePicbilleLooseTenBlockHeight() {
  return PICBILLE_UNIT_SIZE * 5 + PICBILLE_UNIT_GAP * 4;
}

function renderPicbilleLooseTensBlocks(count, x, y) {
  const layout = getPicbilleLooseTensLayout(count);
  if (!layout.count) return "";

  return Array.from({ length: layout.count }, (_, blockIndex) => {
    const blockX = x + blockIndex * (layout.blockWidth + PICBILLE_SMALL_GROUP_GAP);
    return renderPicbilleLooseTenBlock(blockX, y);
  }).join("");
}

function renderPicbilleLooseTenBlock(x, y) {
  const rowGap = PICBILLE_UNIT_GAP;
  const columnGap = PICBILLE_UNIT_GAP;
  const bottomY = y + computePicbilleLooseTenBlockHeight();
  return Array.from({ length: 10 }, (_, index) => {
    const column = Math.floor(index / 5);
    const rowFromBottom = index % 5;
    const unitX = x + column * (PICBILLE_UNIT_SIZE + columnGap);
    const unitY = bottomY - PICBILLE_UNIT_SIZE - rowFromBottom * (PICBILLE_UNIT_SIZE + rowGap);
    return renderPicbilleUnitAt(unitX, unitY, index);
  }).join("");
}

function getPicbilleVerticalUnitsLayout(count, options = {}) {
  const safeCount = clampInt(count, 0, 9);
  if (!safeCount) return { count: 0, width: 0, height: 0 };
  if (options.twoColumns === true) {
    return {
      count: safeCount,
      width: computePicbilleLooseTenBlockWidth(),
      height: computePicbilleLooseTenBlockHeight()
    };
  }
  return {
    count: safeCount,
    width: PICBILLE_UNIT_SIZE,
    height: computePicbilleVerticalUnitsHeight(safeCount)
  };
}

function computePicbilleVerticalUnitsHeight(count) {
  if (count <= 0) return 0;
  let height = count * PICBILLE_UNIT_SIZE;
  for (let index = 0; index < count - 1; index += 1) {
    height += index === 4 ? PICBILLE_UNIT_GROUP_GAP : PICBILLE_UNIT_GAP;
  }
  return height;
}

function renderPicbilleVerticalUnits(count, x, y, options = {}) {
  const safeCount = clampInt(count, 0, 9);
  if (options.twoColumns === true) {
    return renderPicbilleUnitColumns(safeCount, x, y);
  }

  const height = computePicbilleVerticalUnitsHeight(safeCount);
  const bottomY = y + height;
  return Array.from({ length: safeCount }, (_, index) => {
    const unitY = bottomY - PICBILLE_UNIT_SIZE - getPicbilleVerticalUnitOffsetFromBottom(index);
    return renderPicbilleUnitAt(x, unitY, index);
  }).join("");
}

function renderPicbilleUnitColumns(count, x, y) {
  const safeCount = clampInt(count, 0, 10);
  const bottomY = y + computePicbilleLooseTenBlockHeight();
  return Array.from({ length: safeCount }, (_, index) => {
    const column = Math.floor(index / 5);
    const rowFromBottom = index % 5;
    const unitX = x + column * (PICBILLE_UNIT_SIZE + PICBILLE_UNIT_GAP);
    const unitY = bottomY - PICBILLE_UNIT_SIZE - rowFromBottom * (PICBILLE_UNIT_SIZE + PICBILLE_UNIT_GAP);
    return renderPicbilleUnitAt(unitX, unitY, index);
  }).join("");
}

function getPicbilleVerticalUnitOffsetFromBottom(index) {
  let offset = 0;
  for (let step = 0; step < index; step += 1) {
    offset += PICBILLE_UNIT_SIZE;
    offset += step === 4 ? PICBILLE_UNIT_GROUP_GAP : PICBILLE_UNIT_GAP;
  }
  return offset;
}

function renderPicbilleUnitAt(x, y, index) {
  const cx = x + PICBILLE_UNIT_SIZE / 2;
  const cy = y + PICBILLE_UNIT_SIZE / 2;
  const cross = index === 2 || index === 7 ? renderPicbilleCross(cx, cy) : "";
  return `${renderInlineAsset({
    x,
    y,
...
function getPicbilleTenGroupsForValue(value) {
  ...
  return getPicbilleTenGroupsForCount(tens, ones);
}

function getPicbilleTenGroupsForCount(tensCount, onesCount = 0) {
  const tens = clampInt(tensCount, 0, 9);
...
function renderPicbillePieceSvg(kind) {
  if (kind === "hundreds") {
    return svgRootExact({
      width: Math.ceil(PICBILLE_HUNDRED_W),
      height: Math.ceil(PICBILLE_HUNDRED_H),
      ariaLabel: "Centaine Picbille",
      body: renderImageAsset({
        x: 0,
        y: 0,
        width: PICBILLE_HUNDRED_W,
        height: PICBILLE_HUNDRED_H,
...
function renderBlueBase10Svg(value) {
  const safeValue = clampInt(value, 1, 999);
  const { hundreds, tens, ones } = decomposeValue(safeValue);
  const cell = 18;
  const unitGap = 6;
  ...
  const hundredsCols = Math.min(hundreds, 5);
  const hundredsRows = hundreds > 0 ? Math.ceil(hundreds / 5) : 0;
  const hundredsWidth = hundreds > 0
    ? hundredsCols * hundredBlockSize + Math.max(0, hundredsCols - 1) * hundredBlockGap
    : 0;
  const hundredsHeight = hundredsRows > 0
    ? hundredsRows * hundredBlockSize + Math.max(0, hundredsRows - 1) * hundredBlockGap
    : 0;
  ...
  const lowerWidth = (tensSection.width || 0) + (tensSection.width && unitsSection.width ? sectionGap : 0) + (unitsSection.width || 0);
  const lowerHeight = Math.max(tensSection.height || 0, unitsSection.height || 0);

  const contentWidth = Math.max(hundredsWidth, lowerWidth, cell);
  const contentHeight = hundreds > 0
    ? hundredsHeight + (lowerHeight > 0 ? sectionGap : 0) + lowerHeight
    : Math.max(lowerHeight, cell);
  const canvasWidth = contentWidth + panelPadX * 2;
  const canvasHeight = contentHeight + panelPadY * 2;

  if (hundreds > 0) {
    const startX = panelPadX + (contentWidth - hundredsWidth) / 2;
    const startY = panelPadY;
    for (let index = 0; index < hundreds; index += 1) {
      const col = index % 5;
      const row = Math.floor(index / 5);
      body.push(renderBlueHundredBlock(
        startX + col * (hundredBlockSize + hundredBlockGap),
        startY + row * (hundredBlockSize + hundredBlockGap),
        cell,
        strokeWidth
      ));
    }
  ...
  if (lowerWidth > 0) {
    const startX = panelPadX + (contentWidth - lowerWidth) / 2;
    const lowerTop = panelPadY + (hundreds > 0 ? hundredsHeight + sectionGap : (contentHeight - lowerHeight) / 2);
    let cursorX = startX;

    if (tensSection.width) {
      body.push(`<g transform="translate(${round(cursorX)},${round(lowerTop + (lowerHeight - tensSection.height) / 2)})">${tensSection.body}</g>`);
      cursorX += tensSection.width;
      if (unitsSection.width) cursorX += sectionGap;
    }

    if (unitsSection.width) {
      body.push(`<g transform="translate(${round(cursorX)},${round(lowerTop + (lowerHeight - unitsSection.height) / 2)})">${unitsSection.body}</g>`);
    }
  }
...
function buildBlueUnitsSection(count, cell, gap, strokeWidth) {
  if (!count) return { width: 0, height: 0, body: '' };
  const width = Math.min(count, 5) * cell + Math.max(0, Math.min(count, 5) - 1) * gap;
  const height = count > 5 ? (2 * cell + gap) : cell;
  const body = Array.from({ length: count }, (_, index) => {
    const col = index % 5;
    const row = Math.floor(index / 5);
    return renderBlueSquare(col * (cell + gap), row * (cell + gap), cell, strokeWidth);
  }).join('');
...
function renderImageAsset({ x, y, width, height, href }) {
  return `
    <image x="${round(x)}" y="${round(y)}" width="${round(width)}" height="${round(height)}" href="${escapeHtml(href)}" preserveAspectRatio="xMidYMid meet" aria-hidden="true" />
  `;
}
