...
function getLineHighlightElements(state) {
  ...
  return Array.from(state.lineSvgEl.querySelectorAll(".rn-svg-tick--target"));
}
...
function syncLineHighlight(state) {
  ...
  svg.querySelectorAll(".rn-svg-tick--target").forEach((tickEl) => {
    const tickIndex = Number(tickEl.dataset.rnTickIndex);
    if (tickIndex === highlightedIndex) return;
    const tick = question.ticks.find((item) => item.index === tickIndex);
    if (tick) {
      const isMarker = question.markerIndices.includes(tick.index);
      tickEl.setAttribute("stroke-width", getGraduatedTickShape(question.lineType, tick.role, isMarker, false).width);
    }
    tickEl.classList.remove("rn-svg-tick--target");
  });

  if (!Number.isInteger(highlightedIndex)) return;
  const nextTick = svg.querySelector(`[data-rn-tick-index="${highlightedIndex}"]`);
  const questionTick = question.ticks.find((item) => item.index === highlightedIndex);
  if (!nextTick || !questionTick) return;
  const isMarker = question.markerIndices.includes(questionTick.index);
  nextTick.setAttribute("stroke-width", getGraduatedTickShape(question.lineType, questionTick.role, isMarker, true).width);
  nextTick.classList.add("rn-svg-tick--target");
}
