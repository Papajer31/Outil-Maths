...
function renderDisplayedResponse(state) {
  ...
  state.responseWrap.classList.toggle("add-response-wrap--incorrect", showStudentAnswer && evaluation.isCorrect !== true);
  destroyAnswerControl(state);
  state.responseWrap.innerHTML = renderNumericAnswerDisplayMarkup(snapshot?.value ?? "", {
    className: `calc-family-response-input add-response-input${showStudentAnswer ? (evaluation.isCorrect ? " calc-family-response-input--correct add-response-input--correct" : " calc-family-response-input--incorrect add-response-input--incorrect") : (!evaluation.isCorrect ? " calc-family-response-input--correction" : " calc-family-response-input--correct add-response-input--correct")}`,
