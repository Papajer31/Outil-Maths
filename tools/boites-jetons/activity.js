...
function updateInstructionDisplay(state) {
  const solutionCount = Math.max(1, Math.min(3, Number(state.minSolutionsToFind) || 3));
  const fallback = `Clique sur les boites pour trouver ${solutionCount} solution${solutionCount > 1 ? "s" : ""}.`;
