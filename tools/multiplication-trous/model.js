...
export const MULTIPLICATION_HOLE_LIMITS = Object.freeze({
  valueMin: 0,
  valueMax: 99999,
  firstMax: 99999,
  secondMax: 999,
...
export function getDefaultSettings() {
  return {
    ...
    factorRanges: {
      f1: createDefaultRange(0, MULTIPLICATION_HOLE_LIMITS.defaultFirstMax),
      f2: createDefaultRange(0, MULTIPLICATION_HOLE_LIMITS.defaultSecondMax)
    },
    resultConstraint: {
      enabled: false,
      range: createDefaultRange(0, MULTIPLICATION_HOLE_LIMITS.resultMax)
    },
    fixedListRaw: ""
  };
}
...
export function normalizeSettings(settings = {}) {
  ...
  const ranges = isPlainObject(base.factorRanges) ? base.factorRanges : {};
  ...
  return {
    ...
    factorRanges: {
      f1: normalizeValueRange(ranges.f1, { defaultMin: 0, defaultMax: MULTIPLICATION_HOLE_LIMITS.defaultFirstMax, inputMax: MULTIPLICATION_HOLE_LIMITS.firstMax }),
      f2: normalizeValueRange(ranges.f2, { defaultMin: 0, defaultMax: MULTIPLICATION_HOLE_LIMITS.defaultSecondMax, inputMax: MULTIPLICATION_HOLE_LIMITS.secondMax })
    },
    resultConstraint: {
      enabled: resultConstraint.enabled === true,
      range: normalizeResultRange(resultConstraint.range, {
        defaultMin: MULTIPLICATION_HOLE_LIMITS.resultMin,
        defaultMax: MULTIPLICATION_HOLE_LIMITS.resultMax
      })
    },
...
export function getImpossibleMessage(settings = {}) {
  ...
  if (cfg.generationMode === GENERATION_MODES.RANDOM) {
    ...
    if (cfg.carryMode === CARRY_MODES.WITH) return "Aucune multiplication à trou avec retenue possible avec ces réglages.";
    if (cfg.carryMode === CARRY_MODES.WITHOUT) return "Aucune multiplication à trou sans retenue possible avec ces réglages.";
  ...
  return "Aucune multiplication à trou possible : la liste fixe ne contient aucun calcul valide.";
}
...
function getRandomStructuralImpossibleMessage(cfg) {
  ...
  if (!firstRange?.valueCount || !secondRange?.valueCount) {
    return "Aucune multiplication à trou possible : au moins une plage de facteurs ne contient aucune valeur.";
  }
  ...
  if (minDigitTotal > MULTIPLICATION_HOLE_LIMITS.maxFactorDigitTotal) {
    return `Configuration impossible : les plus petits facteurs possibles ont déjà ${minDigitTotal} chiffres au total, alors que la limite est de ${MULTIPLICATION_HOLE_LIMITS.maxFactorDigitTotal}.`;
  }

  const resultMax = getEffectiveResultMax(cfg, MULTIPLICATION_HOLE_LIMITS.resultMax);
...
function buildQuestionFromValues(first, second) {
  ...
  if (safeFirst < MULTIPLICATION_HOLE_LIMITS.valueMin || safeFirst > MULTIPLICATION_HOLE_LIMITS.firstMax) return null;
  if (safeSecond < MULTIPLICATION_HOLE_LIMITS.valueMin || safeSecond > MULTIPLICATION_HOLE_LIMITS.secondMax) return null;
  if (countIntegerDigits(safeFirst) + countIntegerDigits(safeSecond) > MULTIPLICATION_HOLE_LIMITS.maxFactorDigitTotal) return null;

  const result = computeResult(safeFirst, safeSecond);
  if (!Number.isFinite(result) || result < MULTIPLICATION_HOLE_LIMITS.resultMin || result > MULTIPLICATION_HOLE_LIMITS.resultMax) return null;
  

  return {
    tool: "multiplication-trous",
...
function passesCarryRule(first, second, carryMode) {
  ...
  const hasCarry = hasMultiplicationCarry(first, second);
...
function parseFixedListLine(line) {
  ...
  const leftParts = leftRaw.split(/[xX×*]/).map((part) => part.trim()).filter(Boolean);
...
function normalizeValueRange(range, {
  defaultMin = 0,
  defaultMax = MULTIPLICATION_HOLE_LIMITS.defaultFirstMax,
  inputMax = MULTIPLICATION_HOLE_LIMITS.valueMax
} = {}) {
...
function normalizeResultRange(range, { defaultMin = 0, defaultMax = MULTIPLICATION_HOLE_LIMITS.resultMax } = {}) {
  return normalizeNumericConstraint(range ?? {}, {
