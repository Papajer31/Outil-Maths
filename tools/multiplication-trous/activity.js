...
function renderShell(state) {
  ...
  container.innerHTML = `
    <div class="tool-runtime tool-runtime--operation-trous op-root opt-root${state.showResponseBox ? " op-root--boxed" : " op-root--free"}">
      ${renderToolInstruction({ id: "opt_instruction" })}
      <div class="tool-stage op-stage${state.showResponseBox ? " op-stage--boxed" : " op-stage--free"}" id="opt_stage">
        ${state.showResponseBox
          ? `<div class="tool-answer-row op-equation opt-equation" id="opt_equation"></div>`
...
function renderInputEquation(state) {
  ...
  state.equationEl.innerHTML = renderEquationMarkup(state.currentQuestion, {
    holeMarkup: `
      <div class="tool-answer-panel op-inline-response-shell opt-inline-response-shell" id="opt_response_shell">
        <div class="tool-answer-row op-response-wrap" id="opt_response_wrap"></div>
      </div>
    `
  });
...
function renderResponseInput(state) {
  ...
  destroyAnswerControl(state);
  state.responseWrap.className = "tool-answer-row op-response-wrap";
  state.responseWrap.innerHTML = "";

  state.answerControl = createNumericAnswerControl({
    id: "opt_response_input",
    className: "op-response-input op-response-input--single",
    ariaLabel: "Réponse",
...
function renderMathPart(value, extraClass = "") {
  return `<div class="tool-big op-expr opt-math-part ${escapeHtml(extraClass)}">${escapeHtml(formatMathValue(value))}</div>`;
}
...
function renderFreePart(value) {
  return `<span class="opt-free-part">${escapeHtml(formatMathValue(value))}</span>`;
}
...
function renderSnapshotMarkup(snapshot, { showStudentAnswer, evaluation }) {
  ...
  return renderNumericAnswerDisplayMarkup(snapshot.value, {
    className: `op-response-input op-response-input--single${showStudentAnswer ? (evaluation.isCorrect ? " is-correct op-response-input--correct" : " is-incorrect op-response-input--incorrect") : ""}`,
...
function syncCompactClass(state) {
  ...
  const length = getCurrentLineDisplayLength(state);
  state.root.classList.toggle("calc-runtime--dense", length >= 30);
  state.root.classList.toggle("calc-runtime--compact", length >= 22 && length < 30);
}
...
function getCurrentAnswerMaxLength(state) {
  const rawValue = state.currentQuestion?.missingValue ?? "";
