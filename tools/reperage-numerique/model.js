...
export function normalizeSettings(settings = {}) {
  ...
  const picbilleBoxCount = clampInt(raw.picbilleBoxCount, 2, 6);
...
export function pickQuestion(settings = {}, previousKey = "") {
  ...
  for (const questionType of cfg.questionTypes) {
    const question = cfg.lineType === LINE_TYPES.PICBILLE
      ? createPicbilleQuestion(cfg, questionType)
      : createGraduatedLineQuestion(cfg, questionType);
