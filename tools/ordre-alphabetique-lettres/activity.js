...
const CURSIVE_LOWER_GROUP_CLASSES = [
  "oa-chip--cursive-lower-base",
  "oa-chip--cursive-lower-ascender",
  "oa-chip--cursive-lower-dt",
  "oa-chip--cursive-lower-f",
  "oa-chip--cursive-lower-descender"
];
const CURSIVE_LOWER_GROUP_CLASS_BY_LETTER = new Map();

"aceimnorsuvwx".split("").forEach((letter) => {
  CURSIVE_LOWER_GROUP_CLASS_BY_LETTER.set(letter, "oa-chip--cursive-lower-base");
});
"bhkl".split("").forEach((letter) => {
  CURSIVE_LOWER_GROUP_CLASS_BY_LETTER.set(letter, "oa-chip--cursive-lower-ascender");
});
"dt".split("").forEach((letter) => {
  CURSIVE_LOWER_GROUP_CLASS_BY_LETTER.set(letter, "oa-chip--cursive-lower-dt");
});
CURSIVE_LOWER_GROUP_CLASS_BY_LETTER.set("f", "oa-chip--cursive-lower-f");
"gjpqyz".split("").forEach((letter) => {
  CURSIVE_LOWER_GROUP_CLASS_BY_LETTER.set(letter, "oa-chip--cursive-lower-descender");
});
...
function setChipLabelContent(chip, value, state) {
  ...
  ranges.forEach((range) => {
    appendChipText(glyph, text.slice(cursor, range.start));
  ...
  appendChipText(glyph, text.slice(cursor));
}
...
function renderCorrectionLane(state, answerItems) {
  ...
  answerItems.forEach((value) => {
    const chip = createCorrectionChip(value, state, "oa-chip--correction-slot");
...
function createCorrectionOverlay(state, originals, laneMap, workspace, { positionsAreLocal = false } = {}) {
  ...
  originals.forEach(({ value, chip: sourceChip, rect }, index) => {
    const clone = createCorrectionChip(value, state, "oa-chip--correction-copy", sourceChip);
...
function createCorrectionChip(value, state, extraClassName = "", sourceChip = null) {
  const chip = sourceChip?.cloneNode?.(true) || document.createElement("div");
  chip.classList.add("oa-chip");
  chip.classList.remove(
    "oa-chip--locked",
    "oa-chip--floating",
    "oa-chip--drag-source-hidden",
    "oa-chip--drag-proxy",
    "oa-chip--correction-slot",
    "oa-chip--animating"
  );
  if (extraClassName) chip.classList.add(extraClassName);
  chip.dataset.value = value;
...
function applyChipTypography(chip, value, state) {
  ...
  chip.classList.remove(...CURSIVE_LOWER_GROUP_CLASSES);

  if (writing === "cursive" && caseValue === "lower") {
    const groupClass = CURSIVE_LOWER_GROUP_CLASS_BY_LETTER.get(String(value ?? "").toLowerCase());
    if (groupClass) chip.classList.add(groupClass);
  }
}
...
function syncAlphabetHelp(slot) {
  ...
  if (writingBtn) writingBtn.textContent = writingValue === "cursive" ? "cursif" : "script";
}
