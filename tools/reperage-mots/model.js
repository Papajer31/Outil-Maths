...
export function getPhonemicSpellingUsage(settings = {}) {
  ...
  for (const target of targets) {
    const words = getPlayableWordsForTarget(target, getEnabledSpellingsForTarget(cfg, target), cfg.relevanceLevel, []);
...
export function pickQuestion(settings = {}, {
  ...
  const distractors = pickSimilarDistractors(chosen.word, distractorCount);
...
function pickSimilarDistractors(targetWord, requestedCount) {
...
function similarityWeight(entry) {
  const score = Math.max(1, Number(entry?.score) || 1);
  const familiarity = Math.max(0, Math.min(100, Number(entry?.familiarity) || 0));
  return Math.pow(score, 2) * (0.85 + familiarity / 500);
}
