...
const GLOBAL_MIN = 1;
export const TOOL_THEME_ID = "blocs_textuels";
export const TOOL_THEME_LABEL = "Tuiles";
...
function renderTextBlocksSvg(value, options = {}) {
  ...
  const displayBuild = getTextBlocksDisplayBuild(safeValue, options);
  ...
  const rows = [
    hundreds > 0
      ? {
          kind: "hundreds",
          width: getTextRowWidth(hundreds, tileWidth, tileGapX, 5, groupGap),
          height: tileHeight
        }
      : null,
    tens > 0
      ? {
          kind: "tens",
          width: getTextRowWidth(tens, tileWidth, tileGapX, 5, groupGap),
          height: tileHeight
        }
      : null,
    looseTens > 0
      ? {
          kind: "looseTens",
          width: looseTens * computeTextLooseTenBlockWidth(tileWidth, tileGapX) + Math.max(0, looseTens - 1) * groupGap,
          height: computeTextLooseTenBlockHeight(tileHeight, tileGapY)
        }
      : null,
    ones > 0
      ? {
          kind: "ones",
          width: getTextRowWidth(ones, tileWidth, tileGapX, 5, groupGap),
          height: tileHeight
        }
      : null
  ].filter(Boolean);
  ...
  const contentWidth = rows.length
    ? Math.max(...rows.map((row) => row.width), tileWidth)
    : tileWidth;
  ...
  const startX = panelPadX;
  let cursorY = panelPadY;
  const bodies = [];

  rows.forEach((row, rowIndex) => {
    if (row.kind === "hundreds") {
      bodies.push(renderTextTileRow({
        x: startX,
        y: cursorY,
        count: hundreds,
        lines: buildTextBlockLines("hundred", labelMode),
        tileWidth,
        tileHeight,
        gapX: tileGapX,
        borderWidth,
    ...
    } else if (row.kind === "tens") {
      bodies.push(renderTextTileRow({
        x: startX,
        y: cursorY,
        count: tens,
        lines: buildTextBlockLines("ten", labelMode),
        tileWidth,
        tileHeight,
        gapX: tileGapX,
        borderWidth,
    ...
    } else if (row.kind === "looseTens") {
      bodies.push(renderTextLooseTenRow({
        x: startX,
        y: cursorY,
        count: looseTens,
        lines: buildTextBlockLines("unit", labelMode),
        tileWidth,
        tileHeight,
        gapX: tileGapX,
        gapY: tileGapY,
    ...
    cursorY += row.height + (rowIndex < rows.length - 1 ? rowGap : 0);
  });
...
function getTextBlocksDisplayBuild(value, options = {}) {
  const decomposition = decomposeValue(value);
...
function getTextRowWidth(count, tileWidth, gapX, groupEvery = 0, groupGap = 0) {
  const safeCount = Math.max(0, Number(count) || 0);
  if (!safeCount) return 0;
  let width = safeCount * tileWidth + Math.max(0, safeCount - 1) * gapX;
  if (groupEvery > 0) {
...
function renderTextTileRow({
  ...
  for (let index = 0; index < count; index += 1) {
    body.push(renderTextTile(cursorX, y, tileWidth, tileHeight, lines, borderWidth));
    cursorX += tileWidth;
    if (index < count - 1) {
      cursorX += gapX;
      if (groupEvery > 0 && (index + 1) % groupEvery === 0) {
        cursorX += groupGap;
      }
    }
  }
...
function renderTextLooseTenRow({
  x,
  y,
  count,
  lines,
  ...
  const blockWidth = computeTextLooseTenBlockWidth(tileWidth, gapX);
  return Array.from({ length: count }, (_, index) => {
    const blockX = x + index * (blockWidth + blockGap);
    return `<g transform="translate(${round(blockX)},${round(y)})">${renderTextLooseTenBlock(lines, tileWidth, tileHeight, gapX, gapY, borderWidth)}</g>`;
  }).join("");
}

function getDisplaySectionRows(count, columns) {
  const rows = [];
...
function renderTextHundredRows(rows, x, y, sectionWidth, tileWidth, tileHeight, gapX, gapY, borderWidth, labelMode) {
  let cursorY = y;
  return rows.map((count, rowIndex) => {
    const rowWidth = count * tileWidth + Math.max(0, count - 1) * gapX;
    let cursorX = x + Math.max(0, (sectionWidth - rowWidth) / 2);
    const body = Array.from({ length: count }, () => {
      const tile = renderTextTile(
        cursorX,
        cursorY,
        tileWidth,
        tileHeight,
        buildTextBlockLines("hundred", labelMode),
        borderWidth
      );
      cursorX += tileWidth + gapX;
      return tile;
    }).join("");
    cursorY += tileHeight + (rowIndex < rows.length - 1 ? gapY : 0);
    return body;
  }).join("");
}

function buildTextTileVerticalSection(count, lines, tileWidth, tileHeight, gapY, groupGap, borderWidth) {
  ...
  groups.forEach((groupCount, groupIndex) => {
    if (groupIndex > 0) {
      cursorY += gapY + groupGap;
    }
    for (let index = 0; index < groupCount; index += 1) {
      body.push(renderTextTile(0, cursorY, tileWidth, tileHeight, lines, borderWidth));
      cursorY += tileHeight;
  ...
  return { width: tileWidth, height: cursorY, body: body.join("") };
}

function buildTextUnitsSection(count, lines, tileWidth, tileHeight, gapX, gapY, groupGap, borderWidth, options = {}) {
  if (!count) return { width: 0, height: 0, body: "" };
  if (options.twoColumns === true) {
    return buildTextUnitColumnsSection(count, lines, tileWidth, tileHeight, gapX, gapY, borderWidth);
  }

  const width = tileWidth;
  const height = tileHeight + getTextVerticalOffsetFromBottom(count - 1, tileHeight, gapY, groupGap);
  const bottomY = height;
  const body = Array.from({ length: count }, (_, index) => {
    const y = bottomY - tileHeight - getTextVerticalOffsetFromBottom(index, tileHeight, gapY, groupGap);
    return renderTextTile(0, y, tileWidth, tileHeight, lines, borderWidth);
  }).join("");

  return { width, height, body };
}

function buildTextUnitColumnsSection(count, lines, tileWidth, tileHeight, gapX, gapY, borderWidth) {
  const safeCount = clampInt(count, 0, 9);
  if (!safeCount) return { width: 0, height: 0, body: "" };
  const width = computeTextLooseTenBlockWidth(tileWidth, gapX);
  const height = computeTextLooseTenBlockHeight(tileHeight, gapY);
  const bottomY = height;
  const body = Array.from({ length: safeCount }, (_, index) => {
    const column = Math.floor(index / 5);
    const rowFromBottom = index % 5;
    const x = column * (tileWidth + gapX);
    const y = bottomY - tileHeight - rowFromBottom * (tileHeight + gapY);
    return renderTextTile(x, y, tileWidth, tileHeight, lines, borderWidth);
  }).join("");
  return { width, height, body };
}

function buildTextLooseTensSection(count, lines, tileWidth, tileHeight, gapX, gapY, groupGap, borderWidth) {
  const safeCount = clampInt(count, 0, 9);
  if (!safeCount) return { width: 0, height: 0, body: "" };
  const blockWidth = computeTextLooseTenBlockWidth(tileWidth, gapX);
  const blockHeight = computeTextLooseTenBlockHeight(tileHeight, gapY);
  const body = Array.from({ length: safeCount }, (_, blockIndex) => {
    const x = blockIndex * (blockWidth + groupGap);
    return `<g transform="translate(${round(x)},0)">${renderTextLooseTenBlock(lines, tileWidth, tileHeight, gapX, gapY, borderWidth)}</g>`;
  }).join("");
  return {
...
function computeTextLooseTenBlockWidth(tileWidth, gapX) {
  return 2 * tileWidth + gapX;
}

function computeTextLooseTenBlockHeight(tileHeight, gapY) {
  return 5 * tileHeight + 4 * gapY;
}

function renderTextLooseTenBlock(lines, tileWidth, tileHeight, gapX, gapY, borderWidth) {
  const bottomY = computeTextLooseTenBlockHeight(tileHeight, gapY);
...
function getTextVerticalOffsetFromBottom(index, tileHeight, gapY, groupGap) {
  let offset = 0;
  for (let step = 0; step < index; step += 1) {
    offset += tileHeight;
    offset += step === 4 ? groupGap : gapY;
  }
  return offset;
}
...
function buildTextBlockLines(kind, labelMode) {
  switch (labelMode) {
    case TEXT_BLOCKS_LABEL_MODES.NUMERIC_VALUE:
      if (kind === "hundred") return ["100"];
      if (kind === "ten") return ["10"];
      return ["1"];
...
function renderTextBlocksPieceSvg(kind, options = {}) {
  ...
  const labelMode = normalizeTextBlocksLabelMode(options.labelMode);
  const previewWidth = clampInt(options.previewWidth ?? 132, 72, 220);
  const previewHeight = clampInt(options.previewHeight ?? 132, 40, 180);
  return svgRootExact({
    width: previewWidth,
    height: previewHeight,
    ariaLabel: `Tuile ${safeKind}`,
    body: renderTextTile(0, 0, previewWidth, previewHeight, buildTextBlockLines(safeKind, labelMode), 1.75)
  });
}
