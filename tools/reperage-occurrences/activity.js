...
"aceimnorsuvwx".split("").forEach((letter) => {
  CURSIVE_LETTER_CLASS_BY_LETTER.set(letter, "ro-item--cursive-lower-base");
});
"bhkl".split("").forEach((letter) => {
  CURSIVE_LETTER_CLASS_BY_LETTER.set(letter, "ro-item--cursive-lower-ascender");
});
"dt".split("").forEach((letter) => {
  CURSIVE_LETTER_CLASS_BY_LETTER.set(letter, "ro-item--cursive-lower-dt");
});
CURSIVE_LETTER_CLASS_BY_LETTER.set("f", "ro-item--cursive-lower-f");
"gjpqyz".split("").forEach((letter) => {
  CURSIVE_LETTER_CLASS_BY_LETTER.set(letter, "ro-item--cursive-lower-descender");
});
...
function renderShell(state) {
  ...
  state.container.innerHTML = `
    <div class="ro-root" data-response-ui="${escapeAttr(state.responseUi)}">
      ${renderToolInstruction({ id:"ro_instruction" })}
      <div class="ro-board" id="ro_board" aria-live="polite"></div>
    </div>
  `;
  state.root = state.container.querySelector(".ro-root");
  state.instructionEl = state.container.querySelector("#ro_instruction");
  state.boardEl = state.container.querySelector("#ro_board");
...
function bindEvents(state) {
  state.boardEl?.addEventListener("click", (event) => {
    ...
    const button = target?.closest("[data-ro-item-id]");
    if (!button || !isQuestionInteractive(state)) return;
    const id = String(button.dataset.roItemId || "").trim();
    if (!id) return;
...
function loadNextQuestion(state) {
  ...
  state.lastRowKey = String(nextQuestion.rowKey || "");
...
function renderQuestion(state) {
  ...
  if (!question) {
    state.boardEl.className = "ro-board is-empty";
    state.boardEl.innerHTML = '<div class="ro-empty-message">L’activité va commencer.</div>';
    return;
  }
  ...
  state.boardEl.className = `ro-board ${lengthClass} ${densityClass}${resultClass}${state.phaseMode === "answer" ? " is-answer" : ""}`;
  state.boardEl.innerHTML = `
    <section class="ro-items" role="group" aria-label="Éléments à examiner">
...
function renderItem(state, item) {
  ...
  if (!interactive) {
    return `<span class="${classes}" data-ro-item-static="${escapeAttr(item.id)}"><span class="ro-item-text">${escapeHtml(item.text)}</span></span>`;
  }
  ...
  return `
    <button
      type="button"
      class="${classes}"
      data-ro-item-id="${escapeAttr(item.id)}"
      aria-pressed="${pressed}"
      aria-label="${escapeAttr(item.text)}"
    ><span class="ro-item-text">${escapeHtml(item.text)}</span></button>
  `;
}

function getItemClassNames(state, item) {
  const classes = ["ro-item", item.writing === "cursive" ? "ro-item--cursive" : "ro-item--script"];
...
function getCursiveLetterClass(value) {
  ...
  if (/^\p{Lu}$/u.test(letter)) return "ro-item--cursive-upper";
...
function syncItemClasses(state) {
  ...
  state.boardEl.querySelectorAll("[data-ro-item-id]").forEach((button) => {
    const id = String(button.dataset.roItemId || "");
...
function updateInstruction(state) {
  ...
  const fallback = "Clique sur toutes les occurrences de la chaîne demandée.";
...
function injectActivityStyles() {
  ...
  const existing = document.querySelector(`link[data-ro-activity-style="${href}"]`);
  if (existing) {
  ...
  stylesReadyPromise = new Promise((resolve) => {
    ...
    link.rel = "stylesheet";
    link.href = href;
    link.dataset.roActivityStyle = href;
