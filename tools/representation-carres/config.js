...
export function renderToolSettings(container, settings) {
  ...
  container.innerHTML = `
    <div class="rd-config-root rdcar-config-root rd-config-root--targeted">
      ${renderToolSettingsStack(
        renderMinMax({
          idPrefix: "rd_numbers",
...
function renderAllowLooseTens(cfg) {
  return `
    <div class="rdcar-loose-tens-wrap${getQuestionTypeMode(cfg) === QUESTION_TYPE_MODES.NUMBER_TO_REPRESENTATION ? " is-hidden" : ""}" data-rdcar-loose-tens-wrap>
      ${renderRadioGroup({
...
function syncAllowLooseTensVisibility(container, questionTypeMode) {
  const wrap = container.querySelector("[data-rdcar-loose-tens-wrap]");
  if (!wrap) return;
