...
export const NUMBER_POOL_OPTIONS = [
  { id: "1", label: "1", values: [1], row: "base" },
  { id: "2", label: "2", values: [2], row: "base" },
  { id: "3", label: "3", values: [3], row: "base" },
  { id: "4", label: "4", values: [4], row: "base" },
  { id: "5", label: "5", values: [5], row: "base" },
  { id: "6", label: "6", values: [6], row: "base" },
  { id: "7", label: "7", values: [7], row: "base" },
  { id: "8", label: "8", values: [8], row: "base" },
  { id: "9", label: "9", values: [9], row: "base" },
  { id: "10", label: "10", values: [10], row: "base" },
  { id: "15", label: "15", values: [15], row: "special" },
  { id: "20", label: "20", values: [20], row: "special" },
  { id: "25", label: "25", values: [25], row: "special" },
  { id: "50", label: "50", values: [50], row: "special" },
  { id: "75", label: "75", values: [75], row: "special" },
  { id: "100", label: "100", values: [100], row: "special" },
  { id: "250", label: "250", values: [250], row: "special" },
  { id: "otherTens", label: "autres dizaines", values: [30, 40, 60, 70, 80, 90], row: "special" }
];

// Alias conservé pour limiter l’écart avec le vieux moteur Nombre cible copié comme point de départ.
export const CLASSIC_SPECIAL_NUMBER_OPTIONS = NUMBER_POOL_OPTIONS;
export const OPERATION_OPTIONS = ["+", "-", "×", "÷"];
export const DEFAULT_ALLOWED_OPERATIONS = ["+", "-", "×"];
export const TARGETED_OPERATIONS = DEFAULT_ALLOWED_OPERATIONS;

const DEFAULT_EXERCISE_TYPE = EXERCISE_TYPES.TARGETED_CALCULATIONS;
...
export function getDefaultSettings() {
  return {
    ...
    targetedCalculations: {
      ...
      specialNumbers: createDefaultNumberPoolSelection()
    },
    classicChallenge: {
      targetMin: 100,
      targetMax: 500,
...
export function normalizeTargetedCalculationsSettings(raw = {}) {
  const base = isPlainObject(raw) ? raw : {};
  const targetRange = normalizeTargetRange(base, { defaultMin: 10, defaultMax: 100, absoluteMax: TARGETED_CALCULATION_RESULT_MAX });
  const allowedOperations = normalizeAllowedOperations(base.allowedOperations, DEFAULT_ALLOWED_OPERATIONS);
  return {
    ...
    specialNumbers: normalizeClassicSpecialNumbers(base.specialNumbers ?? base.numberPool ?? base)
  };
}

export function normalizeClassicChallengeSettings(raw = {}) {
  const base = isPlainObject(raw) ? raw : {};
  const targetRange = normalizeTargetRange(base, { defaultMin: 100, defaultMax: 500, absoluteMax: TARGETED_CALCULATION_RESULT_MAX });
  const allowedOperations = normalizeAllowedOperations(base.allowedOperations, DEFAULT_ALLOWED_OPERATIONS);
  return {
    targetMin: targetRange.min,
    targetMax: targetRange.max,
...
function normalizeTargetRange(raw = {}, { defaultMin = 10, defaultMax = 100, absoluteMax = TARGETED_CALCULATION_RESULT_MAX } = {}) {
  const base = isPlainObject(raw) ? raw : {};
  let min = clampInt(base.targetMin ?? base.min, 1, absoluteMax);
  let max = clampInt(base.targetMax ?? base.max, 1, absoluteMax);
  if (!Number.isFinite(Number(base.targetMin ?? base.min))) min = defaultMin;
  if (!Number.isFinite(Number(base.targetMax ?? base.max))) max = defaultMax;
  min = clampInt(min, 1, absoluteMax);
  max = clampInt(max, 1, absoluteMax);
  if (min > max) [min, max] = [max, min];
  return { min, max };
}

function normalizeAllowedOperations(value, fallback = DEFAULT_ALLOWED_OPERATIONS) {
  const source = Array.isArray(value) ? value : fallback;
  const out = [];
  source.forEach((op) => {
    const normalized = normalizeOperation(op);
    if (!normalized) return;
    if (normalized === "÷" && !out.includes("÷")) {
      out.push("÷");
      return;
    }
    if (OPERATION_OPTIONS.includes(normalized) && !out.includes(normalized)) {
      out.push(normalized);
    }
  });
...
function createDefaultNumberPoolSelection() {
  return NUMBER_POOL_OPTIONS.reduce((acc, option) => {
    acc[option.id] = option.id !== "250" && option.id !== "otherTens";
    return acc;
  }, {});
}
...
export function normalizeClassicSpecialNumbers(raw = {}) {
  const base = isPlainObject(raw) ? raw : {};
  const defaults = createDefaultNumberPoolSelection();
  const out = {};
...
function buildTargetedCalculationCandidate(cfg) {
  ...
  const numbers = pickTargetedNumbers(settings.numberCount, settings);
  ...
  for (const entry of builders) {
    ...
    if (!Number.isFinite(target) || target < settings.targetMin || target > settings.targetMax) continue;
    if (!Array.isArray(candidate.steps) || candidate.steps.length !== settings.numberCount - 1) continue;
    if (!areStepsAllowed(candidate.steps, settings.allowedOperations)) continue;
...
function solveClassicChallenge(numbers, options = {}) {
  const targetMin = clampInt(options.targetMin, 1, TARGETED_CALCULATION_RESULT_MAX);
  const targetMax = clampInt(options.targetMax, 1, TARGETED_CALCULATION_RESULT_MAX);
  ...
  for (let i = 0; i < attempts; i += 1) {
    ...
    if (!Number.isFinite(target) || target < targetMin || target > targetMax) continue;
...
function buildClassicRandomSolution(numbers, options = {}) {
  ...
  const mustUseSpecial = Boolean(options.mustUseSpecial);
  const allowExactDivision = Boolean(options.allowExactDivision);
  const allowedOperations = normalizeAllowedOperations(options.allowedOperations, DEFAULT_ALLOWED_OPERATIONS);
  ...
  while (active.length > 1) {
    ...
    const combined = pickClassicCombination(left, right, { allowExactDivision, allowedOperations });
...
function pickClassicCombination(left, right, options = {}) {
  const allowExactDivision = Boolean(options.allowExactDivision);
  const allowedOperations = normalizeAllowedOperations(options.allowedOperations, DEFAULT_ALLOWED_OPERATIONS)
    .filter((op) => op !== "÷" || allowExactDivision);
  const weighted = [];
  allowedOperations.forEach((op) => {
    const weight = op === "×" ? 2 : 1;
    for (let i = 0; i < weight; i += 1) weighted.push(op);
  });
  const operations = shuffle(weighted.length ? weighted : DEFAULT_ALLOWED_OPERATIONS);
...
function pickClassicChallengeNumbers(settings) {
  ...
  const pool = getEnabledNumberPool(cfg.specialNumbers);
...
function getClassicSpecialValues(settings) {
  const cfg = normalizeClassicChallengeSettings(settings);
  return getEnabledNumberPool(cfg.specialNumbers).filter((value) => value > 10 || value === 15 || value === 20 || value === 25 || value === 50 || value === 75 || value === 100 || value === 250);
}

function pickTargetedNumbers(count, settings) {
  const cfg = normalizeTargetedCalculationsSettings(settings);
  const pool = getEnabledNumberPool(cfg.specialNumbers);
  return shuffle(pool).slice(0, count).sort((a, b) => a - b);
}

function getEnabledNumberPool(selection = {}) {
  const cfg = normalizeClassicSpecialNumbers(selection);
  const values = [];
  NUMBER_POOL_OPTIONS.forEach((option) => {
    if (!cfg[option.id]) return;
...
function areStepsAllowed(steps = [], allowedOperations = DEFAULT_ALLOWED_OPERATIONS) {
  const allowed = new Set(normalizeAllowedOperations(allowedOperations, DEFAULT_ALLOWED_OPERATIONS));
  return (Array.isArray(steps) ? steps : []).every((step) => allowed.has(normalizeOperation(step?.op)));
}
