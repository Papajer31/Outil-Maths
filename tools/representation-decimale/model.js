...
const CUE_ASSET_URLS = Object.freeze({
  picbille: new URL("./assets/picbille.webp", import.meta.url).href,
  dede: new URL("./assets/dede.webp", import.meta.url).href,
  blocs_textuels: new URL("./assets/tuiles.webp", import.meta.url).href,
  blocs_bleus_base10: new URL("./assets/blue_squares.webp", import.meta.url).href
});
...
export const REPRESENTATION_THEMES = Object.freeze([
  Object.freeze({ id: "picbille", label: "Picbille", max: 99 }),
  Object.freeze({ id: "dede", label: "Dédé", max: 99 }),
  Object.freeze({ id: "blocs_bleus_base10", label: "Petits carrés", max: 999 }),
  Object.freeze({ id: "blocs_textuels", label: "Tuiles", max: 999 })
]);

const THEME_BY_ID = Object.freeze(Object.fromEntries(REPRESENTATION_THEMES.map((theme) => [theme.id, theme])));
const DEFAULT_ACTIVE_THEMES = Object.freeze(["blocs_bleus_base10", "blocs_textuels"]);
...
export function getCompatibleThemeIdsForValue(value) {
  ...
  return REPRESENTATION_THEMES.filter((theme) => safeValue <= theme.max).map((theme) => theme.id);
}
...
function buildQuestionCandidate(settings, values, directions) {
  ...
  return {
    id: `${value}_${direction}_${themeId}`,
    value,
    n: value,
    direction,
    themeId,
...
function pickThemeForValue(settings, value) {
  const compatible = new Set(getCompatibleThemeIdsForValue(value));
  const themeIds = normalizeSettings(settings).activeThemes.filter((themeId) => compatible.has(themeId));
  return themeIds.length ? pickRandom(themeIds) : null;
}

function normalizeActiveThemes(rawThemes, max) {
  const requested = Array.isArray(rawThemes) ? rawThemes : rawThemes == null ? [] : [rawThemes];
  const compatibleSet = new Set(REPRESENTATION_THEMES.filter((theme) => max <= theme.max).map((theme) => theme.id));
  const normalized = [];

  requested.forEach((themeId) => {
    const safeThemeId = String(themeId || "").trim();
    if (!compatibleSet.has(safeThemeId)) return;
    if (normalized.includes(safeThemeId)) return;
    normalized.push(safeThemeId);
  });

  if (!normalized.length) {
    const fallback = REPRESENTATION_THEMES.find((theme) => compatibleSet.has(theme.id));
    if (fallback) {
