...
export const ADDITION_HOLE_LIMITS = Object.freeze({
  valueMin: 0,
  valueMax: 999999,
  defaultFirstMax: 99,
  defaultSecondMax: 99,
...
export function getDefaultSettings() {
  return {
    ...
    termRanges: {
      t1: createDefaultRange(0, ADDITION_HOLE_LIMITS.defaultFirstMax),
      t2: createDefaultRange(0, ADDITION_HOLE_LIMITS.defaultSecondMax)
    },
    resultConstraint: {
      enabled: false,
      range: createDefaultRange(0, ADDITION_HOLE_LIMITS.resultMax)
    },
    fixedListRaw: ""
  };
}
...
export function normalizeSettings(settings = {}) {
  ...
  return {
    ...
    termRanges: {
      t1: normalizeValueRange(ranges.t1, { defaultMin: 0, defaultMax: ADDITION_HOLE_LIMITS.defaultFirstMax }),
      t2: normalizeValueRange(ranges.t2, { defaultMin: 0, defaultMax: ADDITION_HOLE_LIMITS.defaultSecondMax })
    },
    resultConstraint: {
      enabled: resultConstraint.enabled === true,
      range: normalizeResultRange(resultConstraint.range, {
        defaultMin: ADDITION_HOLE_LIMITS.resultMin,
        defaultMax: ADDITION_HOLE_LIMITS.resultMax
      })
    },
...
export function getImpossibleMessage(settings = {}) {
  ...
  if (cfg.generationMode === GENERATION_MODES.RANDOM) {
    ...
    if (cfg.carryMode === CARRY_MODES.WITH) return "Aucune addition à trou avec retenue possible avec ces réglages.";
    if (cfg.carryMode === CARRY_MODES.WITHOUT) return "Aucune addition à trou sans retenue possible avec ces réglages.";
  ...
  return "Aucune addition à trou possible : la liste fixe ne contient aucun calcul valide.";
}
...
function getRandomStructuralImpossibleMessage(cfg) {
  ...
  if (!firstRange?.valueCount || !secondRange?.valueCount) {
    return "Aucune addition à trou possible : au moins une plage de termes ne contient aucune valeur.";
  }

  const resultMax = getEffectiveResultMax(cfg, ADDITION_HOLE_LIMITS.resultMax);
  const minResult = Math.floor(Number(firstRange.min) || 0) + Math.floor(Number(secondRange.min) || 0);
...
function buildQuestionFromValues(first, second) {
  ...
  if (safeFirst < ADDITION_HOLE_LIMITS.valueMin || safeFirst > ADDITION_HOLE_LIMITS.valueMax) return null;
  if (safeSecond < ADDITION_HOLE_LIMITS.valueMin || safeSecond > ADDITION_HOLE_LIMITS.valueMax) return null;

  const result = computeResult(safeFirst, safeSecond);
  if (!Number.isFinite(result) || result < ADDITION_HOLE_LIMITS.resultMin || result > ADDITION_HOLE_LIMITS.resultMax) return null;
...
function passesCarryRule(first, second, carryMode) {
  ...
  const hasCarry = hasAdditionCarry(first, second);
...
function normalizeValueRange(range, { defaultMin = 0, defaultMax = ADDITION_HOLE_LIMITS.defaultFirstMax } = {}) {
...
function normalizeResultRange(range, { defaultMin = 0, defaultMax = ADDITION_HOLE_LIMITS.resultMax } = {}) {
  return normalizeNumericConstraint(range ?? {}, {
